...
impl InstrumentSeparator {
    ...
    pub fn separate(
        ...
        // Check if we already have stems in the cache (with version validation).
        // The version is bumped when the separation algorithm changes, so stale
        // caches from older versions are automatically discarded.
        const CACHE_VERSION: u32 = 3; // v1=Python, v2=ONNX Rust, v3=model selection fix
        ...
        } else if stem_cache_root.exists() {
            // Stale cache from a previous version — clear it.
            println!("[DEBUG] Clearing stale stem cache (expected version {CACHE_VERSION})");
            let _ = std::fs::remove_dir_all(&stem_cache_root);
        }

        std::fs::create_dir_all(&stem_cache_root)?;
        let _ = std::fs::write(&version_path, CACHE_VERSION.to_string());

        // Set up the progress callback early so the UI shows progress
        // during model loading (which can take 10-30s for a 285MB model).
        let progress_cb = progress_callback.as_ref().map(|cb| {
        ...
        let mode = if separator.using_cpu() { "CPU" } else { "GPU (CUDA)" };
        ...
        let stems = separator.separate_file(source_path, progress_cb)?;

        if stems.is_empty() {
            return Err(anyhow!("No stems were generated by Demucs"));
        }

        // Persist stems as WAV files in the cache so re-opening loads instantly.
        for stem in &stems {
            let stem_filename = format!("{}.wav", stem.stem_type.display_name().to_lowercase());
            let wav_path = stem_cache_root.join(&stem_filename);
            let _ = write_stereo_wav(
                &wav_path,
                &stem.samples_interleaved,
                stem.channels.max(1),
                stem.sample_rate,
            );
        }
...
/// Write interleaved f32 samples to a 16-bit PCM WAV file for stem caching.
fn write_stereo_wav(path: &Path, samples: &[f32], channels: u16, sample_rate: u32) -> Result<()> {
